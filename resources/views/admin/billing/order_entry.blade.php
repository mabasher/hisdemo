                    ...
                    <div class="col-md-9">
                        <div class="card w-100 p-3">
                            <div class="row">
                                <div class="col">
                                    <label for="">PID</label>
                                    <input type="text" id="pid" class="form-control" name="reg_no" placeholder="Search PID" required autofocus autocomplete="off">
                                </div>
                                <div class="col">
                                    <label for="">CID</label>
                                    <input type="text" id="cid" class="form-control" name="consult_no" readonly placeholder="CID">
                                </div>
                                <div class="col">
                            ...
                            <div class="row">
                                ...
                                <div class="col">
                                    ...
                                    <input type="text" class="form-control" name="ref_by" readonly placeholder="Referred by">
                                </div>
                                <div class="col">
                            ...
                            <div class="card">
                                <div class="card-body border border-info">
                                    ...
                                    <div class="form-group row">
                                        ...
                                        <div class="col-md-8">
                                            <input type="number" class="form-control grantTotal" value="0" readonly id="grantTotal"  placeholder="Grant Total" />
                                        </div>
                                    </div>
                                    <div class="form-group row">
                                        <label for="grantTotal" class="col-sm-4 col-form-label col-form-label-sm">Receivable Amount</label>
                                        <div class="col-md-8">
                                            <input type="number" class="form-control receivableAmt" name="receivable_amt" value="0" readonly id="receivableAmt"  placeholder="Grant Total" />
                                        </div>
                                    </div>
                                    <div class="form-group row">
                                    ...
                                    <div class="form-group row">
                                        <label for="discount" class="col-sm-4 col-form-label col-form-label-sm">Return Amount</label>
                                        <div class="col-md-8">
                                            <input type="number" class="form-control" readonly id="returnAmt" placeholder="Return Amount" />
                                        </div>
                                    </div>
                                    <div class="form-group row">
                                        <label for="discount" class="col-sm-4 col-form-label col-form-label-sm">Due Amount</label>
                                        <div class="col-md-8">
...
<script>
    ...
    $(document).on('keyup', '#overalldiscount', function() {
        getDiscount('#overalldiscount');
    });

    $(document).on('keyup', '#overalldisPercent', function() {
        getDiscount('#overalldisPercent');
    });

    $(document).on('change', '#overalldisPercent', function() {
        let disPercent = $(this).val();
        let grantTotal = $('#grantTotal').val();
        if (disPercent > 100) {
            alert('Discount Must be less than 100% or Less');
            $(this).val('');
            $("#paidAmt").val(0);
            $("#receivableAmt").val(grantTotal);
            $("#returnAmt").val(0);
            $("#dueAmt").val(0);
        }
    });

    $(document).on('change', '#overalldiscount', function() {
        let disFix = $(this).val();
        let grantTotal = $('#grantTotal').val();
        if (disFix > grantTotal) {
            alert('Discount Must be less Total Amount or Less');
            $(this).val('');
            $("#paidAmt").val(0);
            $("#receivableAmt").val(grantTotal);
            $("#returnAmt").val(0);
    ...
    function getDiscount2(discountId) {
        let discount = 0;
        let totalAmt = 0;
        let receivableAmt = 0;
        let paidAmt = 0;
        let disPercent = 0;
        let disval = $(discountId).val();
        ...
        if (discountId == '#overalldisPercent') {
    ...
    function getDiscount(discountId) {
        let disPercent = $('#overalldisPercent').val();
        let disAmount = $('#overalldiscount').val();
        let totalAmt = parseInt($('#grantTotal').val());
        let discount = 0;
        let rcvAmt = $('#receiveAmt').val();
        let changAmt = 0;
        let dueAmt = 0;
        ...
        if (discountId == '#overalldisPercent') {
            ...
            discount = disAmount + (totalAmt - disAmount) * disPercent / 100;
            receivableAmt = totalAmt - discount;
        } else {
            discount = (totalAmt * disPercent / 100) + disAmount;
            receivableAmt = totalAmt - discount;
        }
        $("#receivableAmt").val(receivableAmt);
        if (rcvAmt > receivableAmt) {
